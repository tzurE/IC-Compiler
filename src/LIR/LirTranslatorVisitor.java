...
public class LirTranslatorVisitor implements LirVisitor {
	...
	public Object visit(Program program, int regCount) {
		...
		for (ICClass icClass : program.getClasses()) {
			if (!icClass.getName().equals("Library")) {
				...
				if (icClass.hasSuperClass()) {
					ClassLayout superClassLayout = this.classLayouts
							.get(icClass.getSuperClassName());
		...
		// here we need to concatenate the 3 lists - literals, program, main.
		// main has to be last!
		// we maybe wont add the main list!!!!!!!!!!!!!!! we found a
		// solution!!!!!!
	...
	public Object visit(StaticMethod method, int regCount) {
		if (!method.getName().equals("main")) {
			...
			LIRNode ic_main_end = new LibraryCall(new Label("__exit"),
					lstOopers, new Reg("Rdummy"));
	...
	public Object visit(Assignment assignment, int regCount) {
		...
		if (assignment.getVariable() instanceof ArrayLocation) {
			ArrayLocation arrayLocation = (ArrayLocation) assignment
					.getVariable();
			Operand arr1 = (Operand) arrayLocation.getArray().accept(this,
					regCount);
			...
			Operand ass = (Operand) assignment.getAssignment().accept(this,
					regCount);
			...
			Operand index = (Operand) arrayLocation.getIndex().accept(this,
					regCount);
			...
			VariableLocation varLocation = (VariableLocation) assignment
					.getVariable();
			String cName = "";
			...
			Operand ass = (Operand) assignment.getAssignment().accept(this,
					regCount);
			SymbolEntry varSym = varLocation.getScope().searchForVar(
			...
			if (varLocation.isExternal()) {// external, lets get some info on
											// it.
				field = true;
				TypeTableType classType = (TypeTableType) varLocation
						.getLocation().accept(
								new SymbolVisitorChecker(this.global),
								varLocation.getScope());
			...
			if (field) {
				...
				LIRNode move1 = new MoveFieldInstr(op, new Immediate(
						fieldOffset), reg1, false);
				temp_Program.add(move1);
			} else {// local!
	...
	public Object visit(Return returnStatement, int regCount) {
		...
		Operand oper = (Operand) returnStatement.getValue().accept(this,
				regCount);
	...
	public Object visit(If ifStatement, int regCount) {
		...
		Operand condOper = (Operand) ifStatement.getCondition().accept(this,
				regCount);
		...
		if (ifStatement.hasElse()) {
			...
			temp_Program
					.add(new CondJumpInstr(new Label(
							LirTranslatorVisitor.falseCondLbl + localNumIf),
							Cond.True));
			ifStatement.getOperation().accept(this, regCount);
			temp_Program.add(new JumpInstr(new Label(
					LirTranslatorVisitor.endCondLbl + localNumIf)));
			temp_Program.add(new Label(LirTranslatorVisitor.falseCondLbl
					+ localNumIf + ":"));
			ifStatement.getElseOperation().accept(this, regCount);
			temp_Program.add(new Label(LirTranslatorVisitor.endCondLbl
					+ localNumIf + ":"));
		} else {
			temp_Program.add(new CondJumpInstr(new Label(
					LirTranslatorVisitor.endCondLbl + localNumIf), Cond.True));
			ifStatement.getOperation().accept(this, regCount);
			temp_Program.add(new Label(LirTranslatorVisitor.endCondLbl
					+ localNumIf + ":"));
		}
	...
	public Object visit(While whileStatement, int regCount) {
		...
		temp_Program.add(new Label(LirTranslatorVisitor.startWhileLbl
				+ localNumOfWhiles + ":"));

		// Translating loop's condition
		Operand condOper = (Operand) whileStatement.getCondition().accept(this,
				regCount);

		temp_Program.add(new CompareInstr(new Immediate(0), condOper));
		temp_Program
				.add(new CondJumpInstr(new Label(
						LirTranslatorVisitor.endWhileLbl + localNumOfWhiles),
						Cond.True));

		whileStatement.getOperation().accept(this, regCount);
		temp_Program.add(new JumpInstr(new Label(
				LirTranslatorVisitor.startWhileLbl + localNumOfWhiles)));
		temp_Program.add(new Label(LirTranslatorVisitor.endWhileLbl
				+ localNumOfWhiles + ":"));
	...
	public Object visit(Break breakStatement, int regCount) {
		...
		LIRNode jump_inst = new JumpInstr(new Label(
				LirTranslatorVisitor.endWhileLbl + this.numOfWhiles));
	...
	public Object visit(LocalVariable localVariable, int regCount) {
		String lName = "v" + localVariable.getScope().getUniqueId()
		...
		if (localVariable.hasInitValue()) {
			Operand oper1 = (Operand) localVariable.getInitValue().accept(this,
					regCount);
	...
	public Object visit(VariableLocation location, int regCount) {
		...
		Operand op = null;
		boolean field = false;
		SymbolEntry sym = location.getScope().searchForVar(name,
				location.getLine());
		if (location.isExternal()) {
			field = true;

			TypeTableType classType = (TypeTableType) location.getLocation()
			...
			while (!(currScope.getType().compareTo(SymbolTableType.CLASS) == 0)) {
				...
				if (currScope.getType()
						.compareTo(SymbolTableType.STATIC_METHOD) == 0
						|| currScope.getType().compareTo(
								SymbolTableType.VIRTUAL_METHOD) == 0) {
					if (currScope.getEntry(name, SymbolKinds.LOCAL_VARIABLE) != null
							|| currScope.getEntry(name, SymbolKinds.PARAMETER) != null) {
						if (sym.getKind().equals(SymbolKinds.PARAMETER))
							return new Memory(location.getName());
						else {
							return new Memory("v" + currScope.getUniqueId()
			...
			if (this.classLayouts.get(className).getFieldByName()
					.containsKey(name)) {
				field = true;
		...
		if (field) {
			...
			LIRNode movef = new MoveFieldInstr(new Memory(class_t),
					new Immediate(fieldOffset), reg3, true);
	...
	public Object visit(StaticCall call, int regCount) {
		...
		if (call.getClassName().equals("Library")) {
			...
			ClassSymbolTable classSymbolTable = this.global.getChildTableList()
					.get(call.getClassName());
		...
		// Not a library method
			...
			ClassSymbolTable classSymbolTable = this.global.getChildTableList()
					.get(call.getClassName());
			MethodSymbolTable methodST = (MethodSymbolTable) classSymbolTable
					.getMethodChildTableList().get(call.getName());
			int numFormals = 1;
			...
			reg = new Reg("R" + regCount++);
			callNode = new LIRInstructions.StaticCall(func, paramPairs,
					(Reg) reg);
		}
	...
	public Object visit(VirtualCall call, int regCount) {
		...
		if (call.isExternal()) {
			...
			TypeTableType classType = (TypeTableType) call.getLocation()
					.accept(new SymbolVisitorChecker(this.global),
							call.getScope());
			className = classType.getName();
		...
		// Collect all params of call
		for (Expression callParam : call.getArguments()) {
			int method_unique = global.findUniqueId(m.getScope().getId(),
					call.getName(), SymbolTableType.VIRTUAL_METHOD);
		...
		Operand reg = new Reg("R" + regCount++);
		LIRNode callNode = new LIRInstructions.VirtualCall((Reg) loc,
				new Immediate(methodoff), paramPairs, (Reg) reg);
		temp_Program.add(callNode);
		return reg;
	}
	...
	public Object visit(NewArray newArray, int regCount) {
		...
		if (oper_size.toString().startsWith("v")) {
			...
			temp_Program
					.add(new BinOpInstr(reg, new Immediate(4), Operator.MUL));
			oper_size = reg;
	...
	public Object visit(MathBinaryOp binaryOp, int regCount) {
		...
		Operand oper1 = (Operand) binaryOp.getFirstOperand().accept(this,
				regCount);
		Operand oper2 = null; // (Operand)binaryOp.getSecondOperand().accept(this,
								// regCount);
		LIRNode move = null;
		...
		if (binaryOp.getOperator().getOperatorString()
				.equals(BinaryOps.PLUS.getOperatorString())) {
			TypeTableType operandsType = (TypeTableType) binaryOp
		...
		} else if (binaryOp.getOperator().getOperatorString()
				.equals(BinaryOps.MINUS.getOperatorString())) {
			binOpNode = new BinOpInstr(oper2, oper1, Operator.SUB);
			temp_Program.add(binOpNode);
		} else if (binaryOp.getOperator().getOperatorString()
				.equals(BinaryOps.MULTIPLY.getOperatorString())) {
			binOpNode = new BinOpInstr(oper2, oper1, Operator.MUL);
			temp_Program.add(binOpNode);
		} else if (binaryOp.getOperator().getOperatorString()
				.equals(BinaryOps.DIVIDE.getOperatorString())) {
			binOpNode = new BinOpInstr(oper2, oper1, Operator.DIV);
			temp_Program.add(binOpNode);
		} else if (binaryOp.getOperator().getOperatorString()
				.equals(BinaryOps.MOD.getOperatorString())) {
	...
	public Object visit(Literal literal, int regCount) {
		...
		if (literal.getType().getDescription()
				.equals(LiteralTypes.INTEGER.getDescription())) {
			int i = Integer.parseInt(literal.getValue().toString());
			oper = new Immediate(i);
			return oper;
		} else if (literal.getType().getDescription()
				.equals(LiteralTypes.STRING.getDescription())) {

			LIRNode strLit = new StringLiteral("str" + (numStringLiterals + 1),
					literal.getValue().toString());

			for (LIRNode str : this.StringLiterals) {
				if (((StringLiteral) str).getValue().equals(
						((StringLiteral) strLit).getValue())) {
					((StringLiteral) strLit).setVar(((StringLiteral) str)
							.getVar());
					isStrLitExist = true;
				}
			}
		...
		// LiteralTypes.
		else if (litType.equals(LiteralTypes.TRUE.getDescription())
				&& litVal.equals(LiteralTypes.TRUE.getValue())) {
			oper = new Immediate(1);
		} else if (litType.equals(LiteralTypes.FALSE.getDescription())
				&& litVal.equals(LiteralTypes.FALSE.getValue())) {
			oper = new Immediate(0);
		} else if (litType.equals(LiteralTypes.NULL)
				&& litVal.equals(LiteralTypes.NULL.getValue())) {
			oper = new Immediate(0);
		}
	...
	public Object visit(ExpressionBlock expressionBlock, int regCount) {
		...
		Operand oper = (Operand) expressionBlock.getExpression().accept(this,
				regCount);
		return oper;
	}
